...
func loadTemplates(dir string) *template.Template {
	...
	t, err := template.New("").ParseFiles(path.Join(dir, "sign_in.html"), path.Join(dir, "error.html"), path.Join(dir, "cococlearsession.html"))
	if err != nil {
...
function logout() {
        ...
        xhr.onload = function () {
          ...
          if (xhr.readyState == 4 && xhr.status == "200") {
            ...
            window.location="mykibana.getcoco.buzz";
            //location.reload();
          } else {
            console.error(users);
          }
